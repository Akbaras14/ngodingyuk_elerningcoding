...
import { androidChapters } from "@/lib/courses/android-data";
...
export default function ChapterPage() {
  ...
  const currentIndex = androidChapters.findIndex((c) => c.id === chapterId);
  const currentChapter = androidChapters[currentIndex];

  // Pagination Logic
  const prevChapter = currentIndex > 0 ? androidChapters[currentIndex - 1] : null;
  const nextChapter = currentIndex < androidChapters.length - 1 ? androidChapters[currentIndex + 1] : null;
  ...
  if (!currentChapter) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50">
        ...
        <Button onClick={() => router.push("/courses/android-studio")}>Kembali ke Menu Utama</Button>
      </div>
    );
  }
      ...
      >
        <div className="h-full flex flex-col">
          ...
          <div className="p-4 border-b border-gray-200 bg-white flex items-center justify-between">
            <Link href="/courses/android-studio" className="flex items-center gap-2 font-bold text-gray-900 hover:text-green-600 transition-colors">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-1">
            {androidChapters.map((chapter, idx) => {
              const isActive = chapter.id === chapterId;
              return (
                <Link 
                  key={chapter.id} 
                  href={`/courses/android-studio/${chapter.id}`}
                  onClick={() => setSidebarOpen(false)}
                ...
                >
                  <span className={`flex items-center justify-center w-6 h-6 rounded text-[10px] ${
                    isActive ? "bg-green-200 text-green-900" : "bg-gray-200 text-gray-500"
                  }`}>
                    {idx + 1}
                  </span>
                  {chapter.title}
      ...
      <main className="flex-1 min-w-0 flex flex-col">
        ...
        <div className="flex-1 w-full max-w-4xl mx-auto p-4 md:p-10 lg:p-12 pb-24">
          ...
          <div className="flex items-center gap-2 text-sm text-gray-500 mb-6">
            ...
            <span>/</span>
            <Link href="/courses/android-studio" className="hover:text-green-600">Android</Link>
            <span>/</span>
          ...
          <article className="prose prose-lg prose-green w-full max-w-none text-gray-700 prose-pre:bg-gray-900 prose-pre:text-white">
            <div dangerouslySetInnerHTML={{ __html: currentChapter.content }} />
          </article>

          {/* Code Example Block (Static for Android as it can't run in browser directly) */}
          {currentChapter.code && (
            <div className="mt-10 bg-gray-900 rounded-xl overflow-hidden shadow-xl border border-gray-800">
              <div className="flex items-center justify-between px-4 py-3 bg-gray-800 border-b border-gray-700">
                ...
                <div className="flex items-center gap-2 px-3 py-1.5 bg-gray-700 text-gray-300 text-xs font-bold rounded-full">
                  <Code2 className="w-3 h-3" /> Java/XML
                </div>
              </div>
          ...
          <div className="mt-16 flex flex-col sm:flex-row items-center justify-between gap-4 pt-8 border-t border-gray-200">
            {prevChapter ? (
              <Button 
                ...
                onClick={() => router.push(`/courses/android-studio/${prevChapter.id}`)}
              >
            ...
            {nextChapter ? (
              <Button 
                className="w-full sm:w-auto h-12 px-6 bg-green-600 hover:bg-green-700 text-white shadow-lg shadow-green-600/20"
                onClick={() => router.push(`/courses/android-studio/${nextChapter.id}`)}
              >
                <div className="text-right">
                  <span className="block text-[10px] text-green-100 uppercase tracking-wider">Selanjutnya</span>
              ...
              <Button 
                ...
                onClick={() => router.push("/certificate/android")}
              >
