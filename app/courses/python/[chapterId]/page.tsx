...
import { pythonChapters } from "@/lib/courses/python-data";
...
export default function ChapterPage() {
  ...
  const currentChapter = pythonChapters[currentIndex];
  ...
  if (!currentChapter) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50">
        ...
        <Button onClick={() => router.push("/courses/python")}>Kembali ke Menu Utama</Button>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col md:flex-row font-sans">
      ...
      >
        <div className="h-full flex flex-col">
          ...
          <div className="p-4 border-b border-gray-200 bg-white flex items-center justify-between">
            <Link href="/courses/python" className="flex items-center gap-2 font-bold text-gray-900 hover:text-blue-600 transition-colors">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-1">
            {pythonChapters.map((chapter, idx) => {
              const isActive = chapter.id === chapterId;
              return (
                <Link 
                  key={chapter.id} 
                  href={`/courses/python/${chapter.id}`}
                  onClick={() => setSidebarOpen(false)}
      ...
      <main className="flex-1 min-w-0 flex flex-col">
        ...
        <div className="flex-1 w-full max-w-4xl mx-auto p-4 md:p-10 lg:p-12 pb-24">
          ...
          <div className="flex items-center gap-2 text-sm text-gray-500 mb-6">
            ...
            <span>/</span>
            <Link href="/courses/python" className="hover:text-blue-600">Python</Link>
            <span>/</span>
          ...
          {currentChapter.code && (
            <div className="mt-10 bg-gray-900 rounded-xl overflow-hidden shadow-xl border border-gray-800">
              <div className="flex items-center justify-between px-4 py-3 bg-gray-800 border-b border-gray-700">
                ...
                <button 
                  // For Python, we can't easily run it in browser without something like Pyodide.
                  // For this scope, we might just copy it or show it. 
                  // But let's reuse the modal for viewing/editing text, even if not running.
                  onClick={() => {
                ...
                >
                  <Play className="w-3 h-3 fill-current" /> View / Edit Code
                </button>
              </div>
          ...
          <div className="mt-16 flex flex-col sm:flex-row items-center justify-between gap-4 pt-8 border-t border-gray-200">
            {prevChapter ? (
              <Button 
                ...
                onClick={() => router.push(`/courses/python/${prevChapter.id}`)}
              >
            ...
            {nextChapter ? (
              <Button 
                ...
                onClick={() => router.push(`/courses/python/${nextChapter.id}`)}
              >
              ...
              <Button 
                ...
                onClick={() => router.push("/certificate/python")}
              >
      ...
      {/* Playground Modal (Simple Text Editor for Python) */}
      {playgroundOpen && (
          ...
          <div className="bg-white w-full max-w-4xl h-[70vh] rounded-2xl overflow-hidden flex flex-col shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-gray-50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-gray-800">Python Viewer</h3>
                  <p className="text-xs text-gray-500">Edit kode Python (Simulasi).</p>
                </div>
              </div>
              <button 
            ...
            <div className="flex-1 flex flex-col min-h-0">
              <div className="bg-gray-100 px-4 py-2 border-b border-gray-200 flex justify-between items-center">
                <span className="text-xs font-mono text-gray-500 font-bold uppercase">main.py</span>
              ...
              <div className="bg-gray-800 px-4 py-3 text-right">
                 <span className="text-xs text-gray-400">Note: Browser cannot execute Python directly without WebAssembly (Pyodide).</span>
              </div>
            </div>
          </div>
        </div>
