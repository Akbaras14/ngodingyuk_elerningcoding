...
import { javascriptChapters } from "@/lib/courses/javascript-data";
...
export default function ChapterPage() {
  ...
  const currentIndex = javascriptChapters.findIndex((c) => c.id === chapterId);
  const currentChapter = javascriptChapters[currentIndex];

  // Pagination Logic
  const prevChapter = currentIndex > 0 ? javascriptChapters[currentIndex - 1] : null;
  const nextChapter = currentIndex < javascriptChapters.length - 1 ? javascriptChapters[currentIndex + 1] : null;
  ...
  if (!currentChapter) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50">
        ...
        <Button onClick={() => router.push("/courses/javascript")}>Kembali ke Menu Utama</Button>
      </div>
    );
  }
      ...
      >
        <div className="h-full flex flex-col">
          ...
          <div className="p-4 border-b border-gray-200 bg-white flex items-center justify-between">
            <Link href="/courses/javascript" className="flex items-center gap-2 font-bold text-gray-900 hover:text-yellow-600 transition-colors">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-1">
            {javascriptChapters.map((chapter, idx) => {
              const isActive = chapter.id === chapterId;
              return (
                <Link 
                  key={chapter.id} 
                  href={`/courses/javascript/${chapter.id}`}
                  onClick={() => setSidebarOpen(false)}
                ...
                >
                  <span className={`flex items-center justify-center w-6 h-6 rounded text-[10px] ${
                    isActive ? "bg-yellow-200 text-yellow-900" : "bg-gray-200 text-gray-500"
                  }`}>
                    {idx + 1}
                  </span>
                  {chapter.title}
      ...
      <main className="flex-1 min-w-0 flex flex-col">
        ...
        <div className="flex-1 w-full max-w-4xl mx-auto p-4 md:p-10 lg:p-12 pb-24">
          ...
          <div className="flex items-center gap-2 text-sm text-gray-500 mb-6">
            ...
            <span>/</span>
            <Link href="/courses/javascript" className="hover:text-yellow-600">JS</Link>
            <span>/</span>
          ...
          <article className="prose prose-lg prose-yellow w-full max-w-none text-gray-700 prose-pre:bg-gray-900 prose-pre:text-white">
            ...
            <div className="mt-10 bg-gray-900 rounded-xl overflow-hidden shadow-xl border border-gray-800">
              <div className="flex items-center justify-between px-4 py-3 bg-gray-800 border-b border-gray-700">
                ...
                <button 
                  ...
                  className="flex items-center gap-2 px-3 py-1.5 bg-yellow-500 hover:bg-yellow-600 text-black text-xs font-bold rounded-full transition-colors"
                >
          ...
          <div className="mt-16 flex flex-col sm:flex-row items-center justify-between gap-4 pt-8 border-t border-gray-200">
            {prevChapter ? (
              <Button 
                ...
                onClick={() => router.push(`/courses/javascript/${prevChapter.id}`)}
              >
            ...
            {nextChapter ? (
              <Button 
                className="w-full sm:w-auto h-12 px-6 bg-yellow-500 hover:bg-yellow-600 text-black shadow-lg shadow-yellow-500/20"
                onClick={() => router.push(`/courses/javascript/${nextChapter.id}`)}
              >
                <div className="text-right">
                  <span className="block text-[10px] text-yellow-900/60 uppercase tracking-wider">Selanjutnya</span>
              ...
              <Button 
                ...
                onClick={() => router.push("/certificate/javascript")}
              >
            ...
            <div className="flex-1 flex flex-col md:flex-row min-h-0">
              ...
              <div className="flex-1 flex flex-col border-r border-gray-200 min-h-0">
                <div className="bg-gray-100 px-4 py-2 border-b border-gray-200 flex justify-between items-center">
                  <span className="text-xs font-mono text-gray-500 font-bold uppercase">JavaScript Source</span>
              ...
              <div className="flex-1 flex flex-col bg-white min-h-0">
                <div className="bg-gray-100 px-4 py-2 border-b border-gray-200 flex justify-between items-center">
                  <span className="text-xs font-mono text-gray-500 font-bold uppercase">Console Output / Result</span>
                ...
                {/* For JS, we iframe it differently or run it. 
                    Simple approach: Use same iframe technique, wrapping JS in <script> tags if not present,
                    or just rendering the whole HTML structure if provided.
                    The sample data provides some HTML context or just JS.
                    Better to wrap input in script tags if it doesn't have them for this simple playground. 
                */}
                <iframe 
                  className="flex-1 w-full bg-white"
                  srcDoc={playgroundCode.includes("<html") || playgroundCode.includes("<script") ? playgroundCode : `
                    <!DOCTYPE html>
                    <html>
                    <body>
                      <div id="output" style="font-family: monospace; padding: 20px;"></div>
                      <script>
                        // Capture console.log to screen
