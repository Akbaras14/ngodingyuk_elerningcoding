...
import { mysqlChapters } from "@/lib/courses/mysql-data";
...
export default function ChapterPage() {
  ...
  const currentIndex = mysqlChapters.findIndex((c) => c.id === chapterId);
  const currentChapter = mysqlChapters[currentIndex];

  // Pagination Logic
  const prevChapter = currentIndex > 0 ? mysqlChapters[currentIndex - 1] : null;
  const nextChapter = currentIndex < mysqlChapters.length - 1 ? mysqlChapters[currentIndex + 1] : null;
  ...
  if (!currentChapter) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50">
        ...
        <Button onClick={() => router.push("/courses/mysql")}>Kembali ke Menu Utama</Button>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col md:flex-row font-sans">
      ...
      >
        <div className="h-full flex flex-col">
          ...
          <div className="p-4 border-b border-gray-200 bg-white flex items-center justify-between">
            <Link href="/courses/mysql" className="flex items-center gap-2 font-bold text-gray-900 hover:text-orange-600 transition-colors">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-1">
            {mysqlChapters.map((chapter, idx) => {
              const isActive = chapter.id === chapterId;
              return (
                <Link 
                  key={chapter.id} 
                  href={`/courses/mysql/${chapter.id}`}
                  onClick={() => setSidebarOpen(false)}
      ...
      <main className="flex-1 min-w-0 flex flex-col">
        ...
        <div className="flex-1 w-full max-w-4xl mx-auto p-4 md:p-10 lg:p-12 pb-24">
          ...
          <div className="flex items-center gap-2 text-sm text-gray-500 mb-6">
            ...
            <span>/</span>
            <Link href="/courses/mysql" className="hover:text-orange-600">MySQL</Link>
            <span>/</span>
          ...
          <article className="prose prose-lg prose-orange w-full max-w-none text-gray-700 prose-pre:bg-gray-900 prose-pre:text-white">
            ...
            <div className="mt-10 bg-gray-900 rounded-xl overflow-hidden shadow-xl border border-gray-800">
              <div className="flex items-center justify-between px-4 py-3 bg-gray-800 border-b border-gray-700">
                <span className="text-xs font-mono text-gray-400 font-bold uppercase tracking-wider">
                  {currentChapter.code.label || "MySQL Script"}
                </span>
                <button 
                  onClick={() => {
                  ...
                  className="flex items-center gap-2 px-3 py-1.5 bg-orange-600 hover:bg-orange-700 text-white text-xs font-bold rounded-full transition-colors"
                >
                  <Play className="w-3 h-3 fill-current" /> View Query
                </button>
              </div>
          ...
          <div className="mt-16 flex flex-col sm:flex-row items-center justify-between gap-4 pt-8 border-t border-gray-200">
            {prevChapter ? (
              <Button 
                ...
                onClick={() => router.push(`/courses/mysql/${prevChapter.id}`)}
              >
            ...
            {nextChapter ? (
              <Button 
                className="w-full sm:w-auto h-12 px-6 bg-orange-600 hover:bg-orange-700 text-white shadow-lg shadow-orange-600/20"
                onClick={() => router.push(`/courses/mysql/${nextChapter.id}`)}
              >
                <div className="text-right">
                  <span className="block text-[10px] text-orange-100 uppercase tracking-wider">Selanjutnya</span>
              ...
              <Button 
                ...
                onClick={() => router.push("/certificate/mysql")}
              >
      ...
      {/* Playground Modal (Static Viewer for MySQL) */}
      {playgroundOpen && (
          ...
          <div className="bg-white w-full max-w-4xl h-[70vh] rounded-2xl overflow-hidden flex flex-col shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-gray-50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-gray-800">MySQL Query Viewer</h3>
                  <p className="text-xs text-gray-500">Lihat dan salin query MySQL.</p>
                </div>
              </div>
              <button 
            ...
            <div className="flex-1 flex flex-col min-h-0">
              <div className="bg-gray-100 px-4 py-2 border-b border-gray-200 flex justify-between items-center">
                <span className="text-xs font-mono text-gray-500 font-bold uppercase">mysql_script.sql</span>
              ...
              <div className="bg-gray-800 px-4 py-3 text-right">
                 <span className="text-xs text-gray-400">Note: Use MySQL Workbench or PHPMyAdmin to run these queries.</span>
              </div>
            </div>
          </div>
        </div>
