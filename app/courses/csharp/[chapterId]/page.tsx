...
import { csharpChapters } from "@/lib/courses/csharp-data";
...
export default function ChapterPage() {
  ...
  const currentIndex = csharpChapters.findIndex((c) => c.id === chapterId);
  const currentChapter = csharpChapters[currentIndex];

  // Pagination Logic
  const prevChapter = currentIndex > 0 ? csharpChapters[currentIndex - 1] : null;
  const nextChapter = currentIndex < csharpChapters.length - 1 ? csharpChapters[currentIndex + 1] : null;
  ...
  if (!currentChapter) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50">
        ...
        <Button onClick={() => router.push("/courses/csharp")}>Kembali ke Menu Utama</Button>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col md:flex-row font-sans">
      ...
      >
        <div className="h-full flex flex-col">
          ...
          <div className="p-4 border-b border-gray-200 bg-white flex items-center justify-between">
            <Link href="/courses/csharp" className="flex items-center gap-2 font-bold text-gray-900 hover:text-purple-600 transition-colors">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-1">
            {csharpChapters.map((chapter, idx) => {
              const isActive = chapter.id === chapterId;
              return (
                <Link 
                  key={chapter.id} 
                  href={`/courses/csharp/${chapter.id}`}
                  onClick={() => setSidebarOpen(false)}
                ...
                >
                  <span className={`flex items-center justify-center w-6 h-6 rounded text-[10px] ${
                    isActive ? "bg-purple-200 text-purple-900" : "bg-gray-200 text-gray-500"
                  }`}>
                    {idx + 1}
                  </span>
                  {chapter.title}
      ...
      <main className="flex-1 min-w-0 flex flex-col">
        ...
        <div className="flex-1 w-full max-w-4xl mx-auto p-4 md:p-10 lg:p-12 pb-24">
          ...
          <div className="flex items-center gap-2 text-sm text-gray-500 mb-6">
            ...
            <span>/</span>
            <Link href="/courses/csharp" className="hover:text-purple-600">C#</Link>
            <span>/</span>
          ...
          <article className="prose prose-lg prose-purple w-full max-w-none text-gray-700 prose-pre:bg-gray-900 prose-pre:text-white">
            ...
            <div className="mt-10 bg-gray-900 rounded-xl overflow-hidden shadow-xl border border-gray-800">
              <div className="flex items-center justify-between px-4 py-3 bg-gray-800 border-b border-gray-700">
                ...
                <button 
                  ...
                  className="flex items-center gap-2 px-3 py-1.5 bg-purple-600 hover:bg-purple-700 text-white text-xs font-bold rounded-full transition-colors"
                >
          ...
          <div className="mt-16 flex flex-col sm:flex-row items-center justify-between gap-4 pt-8 border-t border-gray-200">
            {prevChapter ? (
              <Button 
                ...
                onClick={() => router.push(`/courses/csharp/${prevChapter.id}`)}
              >
            ...
            {nextChapter ? (
              <Button 
                className="w-full sm:w-auto h-12 px-6 bg-purple-600 hover:bg-purple-700 text-white shadow-lg shadow-purple-600/20"
                onClick={() => router.push(`/courses/csharp/${nextChapter.id}`)}
              >
                <div className="text-right">
                  <span className="block text-[10px] text-purple-100 uppercase tracking-wider">Selanjutnya</span>
              ...
              <Button 
                ...
                onClick={() => router.push("/certificate/csharp")}
              >
      ...
      {/* Playground Modal (Static Viewer for C#) */}
      {playgroundOpen && (
          ...
          <div className="bg-white w-full max-w-4xl h-[70vh] rounded-2xl overflow-hidden flex flex-col shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-gray-50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-gray-800">C# Code Viewer</h3>
                  <p className="text-xs text-gray-500">Lihat dan salin kode C#.</p>
                </div>
              </div>
              <button 
            ...
            <div className="flex-1 flex flex-col min-h-0">
              <div className="bg-gray-100 px-4 py-2 border-b border-gray-200 flex justify-between items-center">
                <span className="text-xs font-mono text-gray-500 font-bold uppercase">Program.cs</span>
              ...
              <div className="bg-gray-800 px-4 py-3 text-right">
                 <span className="text-xs text-gray-400">Note: C# requires .NET Runtime to execute.</span>
              </div>
            </div>
          </div>
        </div>
